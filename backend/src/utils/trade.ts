 ...
 */
/* Fetch all open positions from Fyers */
export async function fetchOpenPositions(broker: FyersBroker): Promise<any[]> {
  ...
  // The orderBook is an array of orders, so we filter out orders with status 'OPEN'
  return positionsResponse.orderBook?.filter((order: any) => order.status === 2) || [];
}

/**
 * Get the next contract based on the current contract's expiry month
 */
export async function getNextContract(expiredSymbolId: string): Promise<any> {
  const expiredSymbol = await Instrument.findById(expiredSymbolId);
  if (!expiredSymbol) {
...
/**
 * Close all open positions before rollover
 */
export async function closeAllPositions(broker: FyersBroker, openPositions: any[]): Promise<void> {
...
/**
 * Open new positions with the new contract after rollover
 */
